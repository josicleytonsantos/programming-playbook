# 🚀 Programming Playbook

🌎 🌎 **Looking for the English version of this repository?** Check out [README-en.md](README-en.md).

Este repositório reúne minhas anotações, exemplos e referências rápidas das linguagens de programação que utilizo.

O objetivo é servir como uma base central de conhecimento, funcionando tanto como material de consulta quanto como um portfólio técnico das tecnologias com as quais possuo experiência.

## 💻 Linguagens

* 🐍 Python
* ⚙️ C++
* 🔷 C#
* 🗄️ SQL
* ☕ Java
* 🅰️ Angular
* 📊 R

## 📂 Organização

Cada linguagem possui sua própria estrutura contendo:

* Conceitos fundamentais
* Sintaxe básica
* Estruturas de dados
* Programação orientada a objetos
* Recursos avançados
* Exemplos práticos
* Cheatsheets

## 🎯 Propósito

Este repositório foi criado para:

* Centralizar conhecimento técnico.
* Facilitar revisões rápidas.
* Armazenar exemplos reutilizáveis.
* Documentar padrões e soluções comuns.
* Servir como referência pessoal para estudo e desenvolvimento.

---

## 👨‍💻 Author

**Josicleyton Santos**

🎓 Engenheiro de Produção e Mestre em Ciência da Computação <br>
📊 Análise de Dados | 💻 Programação | ⚙️ Otimização

📧 [santos.josicleyton@gmail.com](mailto:santos.josicleyton@gmail.com) <br>
💼 LinkedIn: linkedin.com/in/josicleyton-santos-0bb43b218